    ...
    db.getConnection(function (err, connection) {
        ...
        async.forEach(clients, function (client, callback) {
            ...
            switch (client.loan_officer) {
                case "Abiodun Atobatele":
                    {
                        client.loan_officer = 2;
                        break;
                    }
                case "Afeez Ishola":
                    {
                        client.loan_officer = 6;
                        break;
                    }
                case "Ayokunnumi Olugbemiro":
                    {
                        client.loan_officer = 3;
                        break;
                    }
                case "Blessing Ebulueye":
                    {
                        client.loan_officer = 5;
                        break;
                    }
                case "Blessing Ebilueye":
                    {
                        client.loan_officer = 5;
                        break;
                    }
                    ...
                    if (error) {
                        ...
                        connection.query('SELECT * from users where ID = LAST_INSERT_ID()', function (err, re, fields) {
                            ...
                            if (!err) {
                                ...
                                res.send(JSON.stringify({
                                    "response": "Error retrieving user details. Please try a new username!"
                                }));
                            }
                        });
                    }
        ...
        connection.query(query2, [req.body.username, req.body.email, req.body.phone], function (error, results, fields) {
            if (results && results[0]) {
                return res.send(JSON.stringify({
                    ...
                    "error": null,
                    "response": results,
                    "message": "Information in use by existing client!"
                }));
            }
...
users.post('/team/members', function (req, res, next) {
    ...
    db.query('SELECT * FROM team_members WHERE teamID=? AND memberID=? AND status = 1', [req.body.teamID, req.body.memberID], function (error, result, fields) {
        if (result && result[0]) {
            res.send({
                "status": 500,
                "error": "User has already been assigned to this team"
            });
        } else {
...
users.get('/user-assigned-target/:id', function (req, res, next) {
    ...
    db.query(query, [req.params.id], function (error, results, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "User assigned target fetched successfully",
                "response": results
            });
        }
    });
        ...
        if (error) {
            ...
            db.query(query2, function (error, user_targets, fields) {
                if (error) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "Targets list fetched successfully",
                        "response": results
                    });
                }
            });
        ...
        if (error) {
            ...
            db.query(query2, [req.params.userID, req.params.targetID], function (error, list, fields) {
                if (error) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "Committals fetched successfully",
                        "response": results
                    });
                }
            });
        ...
        if (error) {
            ...
            db.query(query2, [req.params.id], function (error, list, fields) {
                if (error) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "Target details fetched successfully",
                        "response": results
                    });
                }
            });
...
users.get('/target/limit/:id', function (req, res, next) {
    ...
    db.query(query, [req.params.id, req.params.id, req.params.id], function (error, results, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Target limit fetched successfully",
                "response": results[0]
            });
        }
    });
...
users.post('/team/targets', function (req, res, next) {
    ...
    db.query('SELECT * FROM user_targets WHERE userID=? AND targetID=? AND periodID=? AND sub_periodID=? AND status = 1',
        [req.body.userID, req.body.targetID, req.body.periodID, req.body.sub_periodID],
        function (error, result, fields) {
            if (result && result[0]) {
                res.send({
                    "status": 500,
                    "error": "Target has already been assigned to this team"
                });
            } else {
                    ...
                    if (error) {
                        ...
                        db.query(query, [req.body.userID], function (error, results, fields) {
                            if (error) {
                                ...
                                res.send({
                                    "status": 200,
                                    "message": "Team target assigned successfully",
                                    "response": results
                                });
                            }
                        });
    ...
    db.query('SELECT * FROM user_targets WHERE userID=? AND targetID=? AND periodID=? AND sub_periodID=? AND status = 1',
        ...
        function (error, result, fields) {
            if (result && result[0]) {
                res.send({
                    "status": 500,
                    "error": "Target has already been assigned to this user"
                });
            } else {
                db.query('SELECT * FROM users WHERE ID=?', [req.body.userID], function (error, user, fields) {
                    if (error) {
                        ...
                        if (user[0]['loan_officer_status'] !== 1)
                            return res.send({
                                "status": 500,
                                "error": "User must be a loan officer"
                            });
                        db.query('INSERT INTO user_targets SET ?', req.body, function (error, result, fields) {
                            if (error) {
                                res.send({
                                ...
                                db.query(query, [req.body.userID], function (error, results, fields) {
                                    if (error) {
                                        ...
                                        res.send({
                                            "status": 200,
                                            "message": "User target assigned successfully",
                                            "response": results
                                        });
                                    }
                                });
    ...
    db.query(query, postData, function (error, results, fields) {
        if (error) {
            ...
            res.send(JSON.stringify({
                "status": 200,
                "error": null,
                "response": "New Vehicle Owner Added!"
            }));
        }
    });
});
...
users.post('/contact', function (req, res) {
    ...
    transporter.sendMail(mailOptions, function (error, info) {
        if (error)
            return res.send({
                "status": 500,
                "message": "Oops! An error occurred while sending feedback",
                "response": error
            });
        return res.send({
            "status": 200,
        ...
        if (error) {
            ...
            db.query(query, function (error, results, fields) {
                if (error) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "Application commented successfully!",
                        "response": results
                    });
                }
            });
                ...
                db.query('UPDATE workflow_processes SET approval_status=? WHERE ID=? AND status=1', [1, last_process[0]['ID']], function (error, status, fields) {
                    if (error) {
                        ...
                        if (!(((process.approver_id).split(',')).includes((user_role).toString())))
                            return res.send({
                                "status": 500,
                                "message": "You do not have authorization rights"
                            });
        ...
        if (error) {
            ...
            db.query('UPDATE workflow_processes SET status=? WHERE ID=?', [0, last_process[0]['ID']], function (error, results, fields) {
                if (error) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "Workflow Process reverted successfully!",
                        "response": null
                    });
                }
            });
...
users.get('/workflow_process/:application_id', function (req, res, next) {
    ...
    db.query(query, [req.params.application_id], function (error, results, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Workflow Process fetched successfully!",
                "response": results
            });
        }
    });
...
users.get('/workflow_process_all/:application_id', function (req, res, next) {
    ...
    db.query(query, [req.params.application_id], function (error, results, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "All Workflow Process fetched successfully!",
                "response": results
            });
        }
    });
            ...
            if (application_id && !stage) {
                db.query('SELECT * FROM workflow_processes WHERE ID = (SELECT MAX(ID) FROM workflow_processes WHERE applicationID=? AND status=1)', [application_id], function (error, application_last_process, fields) {
                    if (application_last_process) {
                        let next_stage_index = stages.map(function (e) {
                                return e.stageID;
                            }).indexOf(parseInt(application_last_process[0]['next_stage'])),
                            current_stage_index = stages.map(function (e) {
                                return e.stageID;
                            }).indexOf(parseInt(application_last_process[0]['current_stage']));
                        if (stages[next_stage_index + 1]) {
                            if (application_last_process[0]['next_stage'] !== stages[next_stage_index + 1]['stageID']) { //current stage must not be equal to next stage
                                callback({
                                    previous_stage: application_last_process[0]['current_stage'],
                                    current_stage: application_last_process[0]['next_stage'],
                                    next_stage: stages[next_stage_index + 1]['stageID'],
                                    approver_id: stages[current_stage_index]['approverID']
                                });
                            } else {
                                if (stages[next_stage_index + 2]) {
                                    callback({
                                        previous_stage: application_last_process[0]['current_stage'],
                                        current_stage: application_last_process[0]['next_stage'],
                                        next_stage: stages[next_stage_index + 2]['stageID'],
                                        approver_id: stages[current_stage_index]['approverID']
                                    });
                                } else {
                                    callback({
                ...
                let previous_stage_index = stages.map(function (e) {
                        return e.stageID;
                    }).indexOf(parseInt(stage['previous_stage'])),
                    current_stage_index = stages.map(function (e) {
                        return e.stageID;
                    }).indexOf(parseInt(stage['current_stage'])),
                ...
                if (stage['next_stage']) {
                    callback({
                        ...
                        approver_id: stages[previous_stage_index]['approverID']
                    });
                    ...
                    if (stage['current_stage'] !== stages[next_stage_index]['stageID']) {
                        callback({
                            previous_stage: stage['previous_stage'],
                            current_stage: stage['current_stage'],
                            next_stage: stages[next_stage_index]['stageID'],
                            approver_id: stages[previous_stage_index]['approverID']
                        });
                    } else {
                        if (stages[next_stage_index + 1]) {
                ...
                callback({
                    current_stage: stages[0]['stageID'],
                    next_stage: stages[1]['stageID']
                });
            }
        } else {
            callback({})
...
users.post('/application/comments/:id/:user_id', function (req, res, next) {
    db.query('SELECT * FROM applications WHERE ID = ?', [req.params.id], function (error, application, fields) {
        if (error) {
            ...
            db.query('INSERT INTO application_comments SET ?', [{
...
users.get('/application/comments/:id', function (req, res, next) {
    db.query('SELECT c.text, c.date_created, u.fullname FROM application_comments AS c, users AS u WHERE c.applicationID = ? AND c.userID=u.ID ORDER BY c.ID desc', [req.params.id], function (error, comments, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Application comments fetched successfully!",
                "response": comments
            });
        }
    })
            ...
            if (error) {
                ...
                async.forEach(invoices, function (invoice, callback) {
                    ...
                    async.forEach(req.body.schedule, function (obj, callback2) {
                        ...
                        res.send({
                            "status": 200,
                            "message": "Application scheduled with " + count + " invoices successfully!",
                            "response": null
                        });
                    });
                });
                            ...
                            connection.query('SELECT * FROM application_schedules WHERE applicationID = ? AND status = 2', [req.params.id], function (error, new_schedule, fields) {
                                if (error) {
                                    ...
                                    async.forEach(new_schedule, function (obj, callback2) {
                                        ...
                                        res.send({
                                            "status": 200,
                                            "message": "Application schedule approved with " + count + " invoices successfully!",
                                            "response": null
                                        });
                                    });
                                }
        ...
        connection.query('SELECT * FROM application_schedules WHERE applicationID = ? AND status = 2', [req.params.id], function (error, new_schedule, fields) {
            if (error) {
                ...
                async.forEach(new_schedule, function (obj, callback2) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "Application schedule with " + count + " invoices deleted successfully!",
                        "response": null
                    });
                });
            }
...
users.get('/application/schedule/:id', function (req, res, next) {
    db.query('SELECT * FROM application_schedules WHERE applicationID = ? AND status = 1', [req.params.id], function (error, schedule, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Application schedule fetched successfully!",
                "response": schedule
            });
        }
    });
...
users.get('/application/edit-schedule-history/:id', function (req, res, next) {
    db.query('SELECT s.ID, s.invoiceID, s.payment_amount, s.interest_amount, s.fees_amount, s.penalty_amount, s.payment_collect_date, s.date_modified, s.modifierID,' +
        's.applicationID, u.fullname AS modified_by FROM edit_schedule_history AS s, users AS u WHERE s.modifierID=u.ID AND invoiceID = ? ORDER BY ID desc', [req.params.id],
        function (error, history, fields) {
            if (error) {
                res.send({
                    "status": 500,
                    "error": error,
                ...
                res.send({
                    "status": 200,
                    "message": "Edit schedule history fetched successfully!",
                    "response": history
                });
            }
        });
...
users.get('/application/schedule-history/write-off/:id', function (req, res, next) {
    db.query('UPDATE application_schedules SET ? WHERE ID = ' + req.params.id, {
        payment_status: 2,
        date_modified: moment().utcOffset('+0100').format('YYYY-MM-DD h:mm:ss a')
    }, function (error, result, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Schedule write off successful!"
            });
        }
    });
});
...
users.get('/application/invoice-history/:id', function (req, res, next) {
    db.query('SELECT s.ID, s.invoiceID, s.payment_amount, s.interest_amount, s.fees_amount, s.penalty_amount, s.payment_date, s.date_created, s.status,' +
        's.applicationID, u.fullname AS agent FROM schedule_history AS s, users AS u WHERE s.agentID=u.ID AND invoiceID = ? ORDER BY ID desc', [req.params.id],
        function (error, history, fields) {
            if (error) {
                res.send({
                    "status": 500,
...
users.post('/application/loancirrus-id/:application_id', function (req, res, next) {
    db.query('UPDATE applications SET loanCirrusID=? WHERE ID=?', [req.body.id, req.params.application_id], function (error, result, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Loan Cirrus ID updated successfully!"
            });
        }
    });
});
                ...
                connection.query('SELECT * FROM application_schedules WHERE applicationID = ? AND status = 1 AND payment_status = 0', [req.params.id], function (error, invoices, fields) {
                    if (error) {
                        ...
                        async.forEach(invoices, function (invoice_obj, callback) {
                            ...
                            res.send({
                                "status": 200,
                                "message": "Application write off successful!"
                            });
                        });
                    }
                });
...
users.post('/application/close/:id', function (req, res, next) {
    ...
    db.query('UPDATE applications SET ? WHERE ID = ' + req.params.id, data, function (error, result, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Application closed successful!"
            });
        }
    });
});
...
users.get('/application/cancel/:id', function (req, res, next) {
    ...
    db.query('UPDATE applications SET ? WHERE ID = ' + req.params.id, data, function (error, result, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Application cancellation successful!"
            });
        }
    });
});
    ...
    db.query('SELECT *, (select role_name from user_roles r where r.id = user_role) as role FROM users WHERE username = ?', username, function (err, rows, fields) {
        ...
        transporter.sendMail(mailOptions, function (error, info) {
            if (error)
                return res.send({
                    "status": 500,
                    "message": "Oops! An error occurred while sending request",
                    "response": error
                });
            return res.send({
                "status": 200,
            ...
            db.query(query2, [loan_officer], function (error, results, fields) {
                if (error) {
                    ...
                    res.send({
                        ...
                        "error": null,
                        "response": items,
                        "message": "All Disbursements pulled!"
                    });
                }
            });
        });
    ...
    db.query(query, function (error, results, fields) {
        if (error) {
            ...
            res.send({
                ...
                "error": null,
                "response": results,
                "message": "All Interests Received pulled!"
            });
        }
    });
});
    ...
    db.query(query, function (error, results, fields) {
        if (error) {
            ...
            res.send({
                ...
                "error": null,
                "response": results,
                "message": "All Overdue Loans pulled!"
            });
        }
    });
});
        ...
        case 'payments':
            ...
            if (officer !== '0' && freq == '2') {
                query = 'SELECT   DATE_FORMAT(payment_date, \'%M, %y\') AS PaymentMonth, DATE_FORMAT(Payment_date, \'%M\') month,\n' +
                    ...
                    'and applicationid in (select id from applications where applications.status <> 0)\n' +
                    'AND (select loan_officer from clients where clients.id = (select userid from applications where applications.id = applicationid)) = ' + officer + '\n' +
                    'AND DATE_FORMAT(Payment_date, \'%Y\') = ' + y 'GROUP BY DATE_FORMAT(Payment_date, \'%M%Y\')'
            }
            //One Officer, Monthly
            if (officer !== '0' && freq == '2' && y == '0') {
                query = 'SELECT   DATE_FORMAT(payment_date, \'%M, %y\') AS PaymentMonth, DATE_FORMAT(Payment_date, \'%M\') month,\n' +
        ...
        db.query(query, {
            ...
            if (error) {
                ...
                res.send(JSON.stringify({
                    "status": 200,
                    "error": null,
                    "response": "New Activity Type Added!"
                }));
            }
        });
    });
...
users.get('/user-commissions/:id', function (req, res, next) {
    ...
    db.query(query, [req.params.id], function (error, results, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "User commissions fetched successfully",
                "response": results
            });
        }
    });
...
users.post('/user-commissions', function (req, res, next) {
    ...
    db.query('SELECT * FROM user_commissions WHERE userID=? AND type=? AND sub_periodID=? AND status = 1',
        [req.body.userID, req.body.type, req.body.sub_periodID],
        function (error, result, fields) {
            if (result && result[0]) {
                res.send({
                    "status": 500,
                    "error": "The " + req.body.type + " commission for the same period has already been assigned to this user"
                });
            } else {
                db.query('SELECT * FROM users WHERE ID=?', [req.body.userID], function (error, user, fields) {
                    if (error) {
                            ...
                            if (error) {
                                ...
                                db.query(query, [req.body.userID], function (error, results, fields) {
                                    if (error) {
                                        ...
                                        res.send({
                                            "status": 200,
                                            "message": "User commission assigned successfully",
                                            "response": results
                                        });
                                    }
                                });
        ...
        if (error) {
            ...
            db.query(query, [req.params.userID], function (error, results, fields) {
                if (error) {
                    ...
                    res.send({
                        "status": 200,
                        "message": "User commission deleted successfully",
                        "response": results
                    });
                }
            });
...
users.get('/commissions-list', function (req, res, next) {
    ...
    db.query(query, function (error, results, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Commissions list fetched successfully",
                "response": results
            });
        }
    });
...
users.post('/commission/payments', function (req, res, next) {
    ...
    db.query('INSERT INTO commission_payments SET ?', data, function (error, result, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Commission payment saved successfully!"
            });
        }
    });
});
...
users.get('/commission/payment-history/:commissionID', function (req, res, next) {
    db.query('SELECT * FROM commission_payments WHERE commissionID = ' + req.params.commissionID, function (error, result, fields) {
        if (error) {
            ...
            res.send({
                "status": 200,
                "message": "Commission payment fetched successfully!",
                response: result
            });
        }
    });
